...
class Database_Admin_DLG_Class(tk.Frame):
    def __init__(self, Database_Obj,  *args, **kwargs):
        ...
        MyLabel(36, self.winfo_toplevel(), title_frame, width=30, text='Database Admin').grid(row=0, column=0)
        ...
        MyButton(24, self.winfo_toplevel(), button_frame, command=self.new_table, text='New\nTable', height=3, width=10).grid(row=1,  padx=5, column=2)
        MyButton(24, self.winfo_toplevel(),button_frame, command=self.delete_table, text='Delete\nTable', height=3, width=10).grid(row=1,  padx=5, column=3)
        MyButton(24, self.winfo_toplevel(),button_frame, command=self.alter_table, text='Alter\nTable', height=3, width=10).grid(row=2,  padx=5, column=2)
        MyButton(24, self.winfo_toplevel(),button_frame, text='Cancel', height=3, width=10, command=self.Cancel).grid(row=2, column=3, padx=5)

        ListScrollCombo(False, 10, 25, font_return(16),button_frame, name='table_list_box').grid(row=1, column=1, pady=30, rowspan=2)
        button_frame.nametowidget('table_list_box').config(width=350, height=450)
    ...
    def Cancel(self):
        self.raise_frame('homescreen')

    def alter_table(self):
        table_list_box = self.nametowidget(
            'button_frame').nametowidget('table_list_box')
    ...
    def populate_table_box(self):
        ...
        table_list_box = self.nametowidget('button_frame').nametowidget('table_list_box')
...
class New_Database_Table_DLG_Class(tk.Frame):
    def __init__(self, Database_Obj, *args, **kwargs):
        ...
        title_frame = tk.Frame(self)
        title_frame.grid(row=0, column=1, pady=30, sticky='news')
        MyLabel(36, self.winfo_toplevel(),  title_frame, width=30,
                 text='New Table').grid(row=0, column=0)


        self.input_frame =tk.Frame(self)
        self.input_frame.grid(row=1, column=1, pady=30, sticky='news')

        MyLabel(24, self.winfo_toplevel(),self.input_frame, text='Table Name').grid(row=0, column=0, pady=20)
        
        #validation_field_name = self.winfo_toplevel().register(MySQL_Field_Name)
        MyEntry(24, self.winfo_toplevel(), self.input_frame, name='table_name', validation_type='DB_string').grid(row=0, column=1, columnspan=2, pady=20)

        MyLabel(24, self.winfo_toplevel(),  self.input_frame, text='Field Name').grid(row=1, column=0, pady=20)

        MyLabel(24, self.winfo_toplevel(),  self.input_frame, text='Field Type').grid(row=1, column=1, pady=20)

        MyLabel(24, self.winfo_toplevel(),  self.input_frame, text='Field\nLength').grid(row=1, column=2, pady=20)

        MyEntry(24, self.winfo_toplevel(), self.input_frame, name='field_name',
        ...
        field_menu.config(font=font_return(24))
 
        self.input_frame.nametowidget(
            field_menu.menuname).config(font=font_return(24))

        MyEntry(24, self.winfo_toplevel(), self.input_frame, name='field_length',
        ...
        self.button_frame.grid(row=3, column=1, pady=30, sticky='news')
        self.button_frame.grid_columnconfigure(0, weight=1)
        self.button_frame.grid_columnconfigure(2, weight=1)

        MyButton(24, self.winfo_toplevel(),self.button_frame, text='Add Field', command=self.Add_Field).grid(row=0, column=1, padx=40)
        MyButton(24, self.winfo_toplevel(),self.button_frame, text='Add Table', name='add_table_button', command=self.Add_Table).grid(row=0, column=2, padx=40)
        MyButton(24, self.winfo_toplevel(),self.button_frame, text='Cancel', command=self.Cancel).grid(row=0, column=3, padx=40)
